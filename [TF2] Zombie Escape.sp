...
public Action AuthorBroadcast(Handle timer) {
    CPrintToChatAll("{lime}[ZE] {default}Zombie Escape by knfc");
}
...
public Action: Event_teamplay_round_win(Handle: event,
    const String: name[], bool: dontBroadcast) {
    isGameStarted = false;
}

public Action: Event_player_spawn(Handle: event,
    const String: name[], bool: dontBroadcast) {
...
public void SetClientSpeedScale(int client) {
    if (IsClientInGame(client)) {
        ...
        if (GetClientTeam(client) == 2) {
            ...
            case TFClass_Soldier:
                TF2Attrib_AddCustomPlayerAttribute(client, "move speed bonus", 1.05);
            case TFClass_Pyro:
                TF2Attrib_AddCustomPlayerAttribute(client, "move speed bonus", 1.05);
            case TFClass_DemoMan:
                TF2Attrib_AddCustomPlayerAttribute(client, "move speed bonus", 1.05);
        ...
        if (GetClientTeam(client) == 3) {
            switch (playerClass) {
            case TFClass_Scout:
                TF2Attrib_AddCustomPlayerAttribute(client, "move speed bonus", 320.0 / 400.0);
            }
        }
    }
}
...
public Action: Event_player_death(Handle: event,
    const String: name[], bool: dontBroadcast) {
    new client = GetClientOfUserId(GetEventInt(event, "userid"));
...
public Action: Event_teamplay_round_start(Handle: event,
    const String: name[], bool: dontBroadcast) {
    isGameStarted = false;
    SetConVarString(defaultTeam, "blue", false);
    for (new i = 1; i <= MaxClients; i++) {
...
public Action: Event_teamplay_setup_finished(Handle: event,
    const String: name[], bool: dontBroadcast) {
    isGameStarted = true;
...
stock int SetWeaponAmmo(const int weapon,
    const int ammo) {
